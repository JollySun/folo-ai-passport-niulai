...
CW2017 在共享 I2C 地址 0x63。初始化先读取 CONFIG 确认在线，再按数据手册要求向 CONFIG 依次写入 `0x30`、`0x00` 退出睡眠并重启；等待 100 ms 后读取唤醒态 VERSION，标准值为 `0xA0`。仓库刻意不写自定义电池 profile，因为开源用户的电池可能不同。
...
SOC 准确度取决于电芯与 profile 的匹配程度。部分板卡在唤醒后的最初数秒会暂时返回 `SOC=0`，但 VCELL 已有效；此时驱动用保守的单节锂电压曲线临时估算，待 SOC 有效后自动恢复使用芯片读数。本驱动结果不等于实验室标定值。若产品需要准确 SOC，必须取得电芯参数、CW2017 数据手册和供应商 profile，并完成完整充放电验证。
...
当前 `partitions.csv` 在 2 MB factory 应用分区后增加 2 MB `recordings` 自定义数据分区，分成两个 1 MB 角色声音槽；每槽内部使用两个 512 KB Bank 原子切换，录制或断电失败时仍保留上一段有效声音。每段最多保存 10 秒 16 kHz/16-bit/单声道 PCM；录音按 1 KB 分块从 I2S 直接写入 Flash，不申请整段录音 RAM。首次从旧分区表升级必须烧录整机合并镜像，只刷应用分区无法创建录音分区。
...
- Audio demo 96 KB 录音堆；牛来应用录音采用约 1 KB 流式缓冲；
