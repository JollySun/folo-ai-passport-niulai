...
This repository serves both AI agents and human developers. Coding, validation, and commit rules live in [AGENTS.md](AGENTS.md); the full hardware context is in the [AI Hardware Development Guide](docs/AI_HARDWARE_DEVELOPMENT_GUIDE.md); `docs/` contains the documentation index. Host logic tests and a full ESP-IDF build run in CI (`.github/workflows/build.yml`) on every push to `main` and on pull requests.
